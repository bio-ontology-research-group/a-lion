...
class KG(object):
    ...
    def load_triples_from_dic(self, edgelist):
        ...
        for edge in edgelist:
            ...
            if self.index_ents.get(line[0]) == None:
                last_c += 1
                self.ents[last_c] = str(line[0])
                self.index_ents[str(line[0])] = last_c
                self.ent_tokens[last_c] = set(str(line[0]).replace('(','').replace(')','').split(' '))
            if self.index_ents.get(str(line[1])) == None:
                last_c += 1
                self.ents[last_c] = str(line[1])
                self.index_ents[str(line[1])] = last_c
                self.ent_tokens[last_c] = set(str(line[1]).replace('(','').replace(')','').split(' '))
            if self.index_rels.get(str(line[2])) == None:
                last_r += 1
                self.rels[last_r] = str(line[2])
                self.index_rels[str(line[2])] = last_r
            h = self.index_ents[str(line[0])]
            t = self.index_ents[str(line[1])]
            r = self.index_rels[str(line[2])]
        ...
        print(len(self.rels),len(self.ents))
        ...
        print("#triples, #ents, #rels:", len(self.triples), self.n_ents, self.n_rels)
    ...
    def load_triples(self, filename, splitter = '\t', line_end = '\n'):
        ...
        for line in open(filename):
            ...
            if self.index_ents.get(line[1]) == None:
                last_c += 1
                self.ents[last_c] = line[1]
                self.index_ents[line[1]] = last_c
                self.ent_tokens[last_c] = set(line[1].replace('(','').replace(')','').split(' '))
            if self.index_rels.get(line[2]) == None:
                last_r += 1
                self.rels[last_r] = line[2]
                self.index_rels[line[2]] = last_r
            h = self.index_ents[line[0]]
            t = self.index_ents[line[1]]
            r = self.index_rels[line[2]]
            triples.append([h, r, t])
            self.triples_record.add((h, r, t))
        self.triples = np.array(triples)
        self.n_ents = last_c + 1
