...
def levenshtein_ratio_and_distance(s, t, ratio_calc=False):
    ...
    for col in range(1, cols):
        for row in range(1, rows):
            if s[row - 1] == t[col - 1]:
                # If the characters are the same in the two strings in a given position [i,j] then
                # the cost is 0
                cost = 0
            else:
    ...
    if ratio_calc:
        ...
        # print(distance) # Uncomment if you want to see the matrix showing how the algorithm
        # computes the cost of deletions,
        # insertions and/or substitutions
        # This is the minimum number of edits needed to convert string a to string b
        return "The strings are {} edits away".format(distance[row][col])


def Onto2KGDL2Vec(ontology_file_path):
    ...
    triples = [(x[1], x[0], x[2]) for x in triples]
...
def Onto2KG(ontology_file_path):
    ...
    newKG = KG()
    newKG.load_triples_from_dict(edgelist)
    return newKG
...
def LexicalMatch(source, target, txt):
    ...
    while (len(alignments) <= min_entities and accepted_ratio > 60):
        ...
        print("Starting pool...")
        Result = [pool.apply_async(lex_ma_from_dic_pool, args=(
            ont1_label2class, ont2_label2class, accepted_ratio, k)) for k in keys]
...
# test
# A = LexicalMatch("/home/alghsm0a/A-LIOn/anatomy-dataset/human.owl", "/home/alghsm0a/A-LIOn/
# anatomy-dataset/mouse.owl","anatomy")
