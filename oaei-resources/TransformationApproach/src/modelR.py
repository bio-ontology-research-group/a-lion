...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.variable_scope("graph"):
            ...
            # Mr1 and Mr2 with dimentions d*d (k=d test)
            self._Mr1 = Mr1 = tf.get_variable(
                name='Mr1', 
                shape=[self.dim, self.dim],
                initializer=orthogonal_initializer(),
                dtype=tf.float32)
            self._Mr2 = Mr2 = tf.get_variable(
                name='Mr2', 
                shape=[self.dim, self.dim],
                initializer=orthogonal_initializer(),
                dtype=tf.float32)
            ...
            self._A_h_index = A_h_index = tf.placeholder(
                ...
                name='A_h_index')
            self._A_hr_index = A_hr_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='A_hr_index')
            ...
            self._A_t_index = A_t_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='A_t_index')
            self._A_tr_index = A_tr_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='A_tr_index')
            self._A_hn_index = A_hn_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='A_hn_index')
            self._A_hrn_index = A_hrn_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='A_hrn_index')
            self._A_tn_index = A_tn_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='A_tn_index')
            self._A_trn_index = A_trn_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='A_trn_index')
            '''
            ...
            #additions for TransR
            A_hr_ent_batch = tf.nn.l2_normalize( tf.matmul(tf.nn.embedding_lookup(ht1, A_h_index),Mr1), 1)
            A_tr_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht1, A_t_index),Mr1), 1)
            A_hrn_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht1,A_hn_index),Mr1), 1)
            A_trn_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht1,A_tn_index),Mr1), 1)

            # This stores hr + r - tr
            A_loss_matrix = tf.subtract(tf.add(A_hr_ent_batch, A_rel_batch), A_tr_ent_batch)
            # This stores hr' + r - tr' for negative samples
            A_neg_matrix = tf.subtract(tf.add(A_hrn_ent_batch, A_rel_batch), A_trn_ent_batch)
            # norm
            ...
            self._B_h_index = B_h_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='B_h_index')
            self._B_hr_index = B_hr_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='B_hr_index')
            ...
            self._B_t_index = B_t_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='B_t_index')
            self._B_tr_index = B_tr_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='B_tr_index')
            self._B_hn_index = B_hn_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='B_hn_index')
            self._B_hrn_index = B_hrn_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='B_hrn_index')
            self._B_tn_index = B_tn_index = tf.placeholder(
                ...
                shape=[self._batch_sizeK],
                name='B_tn_index')
            self._B_trn_index = B_trn_index = tf.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK],
                name='B_trn_index')
            ...
            B_hr_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht2, B_h_index),Mr2), 1)
            B_tr_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht2, B_t_index),Mr2), 1)
            B_hrn_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht2,B_hn_index),Mr2), 1)
            B_trn_ent_batch = tf.nn.l2_normalize(tf.matmul(tf.nn.embedding_lookup(ht2,B_tn_index),Mr2), 1)
            
            # This stores hr + r - tr
            B_loss_matrix = tf.subtract(tf.add(B_hr_ent_batch, B_rel_batch), B_tr_ent_batch)
            # This stores hr' + r - tr' for negative samples
            B_neg_matrix = tf.subtract(tf.add(B_hrn_ent_batch, B_rel_batch), B_trn_ent_batch)
            # norm
            # [||h M_hr + r - t M_tr|| + m1 - ||h' M_hr + r - t' M_tr||)]+     here [.]+ means max (. , 0)
