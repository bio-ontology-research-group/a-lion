# This code is a modification of MTransE method to enable learning alignment based on different
# relations on ontologies
# ----------------------------------------------------
...
class Trainer(object):
    ...
    def gen_KM_batch(self, KG_index, forever=False, shuffle=True):
        ...
        if KG_index == 2:
            KG = self.multiG.KG2
        length = KG.triples.shape[0]
        while True:
            triples = KG.triples
            if shuffle:
                np.random.shuffle(triples)
            for i in range(0, length, self.batch_sizeK):
                batch = triples[i: i + self.batch_sizeK, :]
                if batch.shape[0] < self.batch_sizeK:
                    b_ = np.concatenate((batch,
                                         self.multiG.triples[:self.batch_size - batch.shape[0]]),
                                        axis=0)
                    batch = b_
                ...
                yield h_batch.astype(np.int64), r_batch.astype(np.int64), \
                    t_batch.astype(np.int64), neg_h_batch.astype(np.int64), \
                    neg_t_batch.astype(np.int64)
            if not forever:
                break

    def gen_AM_batch(self, forever=False, shuffle=True):
        multiG = self.multiG
        length = len(multiG.align)
        while True:
            align = multiG.align
            if shuffle:
                np.random.shuffle(align)
            for i in range(0, length, self.batch_sizeA):
                batch = align[i: i + self.batch_sizeA, :]
                if batch.shape[0] < self.batch_sizeA:
                    batch = np.concatenate(
                        (batch, align[:self.batch_sizeA - batch.shape[0]]), axis=0)
                    assert batch.shape[0] == self.batch_sizeA
                n_batch = multiG.corrupt_align_batch(batch)
                e1_batch, e2_batch, e1_nbatch, e2_nbatch = batch[:, 0], batch[:, 1], \
                    n_batch[:, 0], n_batch[:, 1]
                yield e1_batch.astype(np.int64), e2_batch.astype(np.int64), \
                    e1_nbatch.astype(np.int64), e2_nbatch.astype(np.int64)
            if not forever:
                break
            ...
            for i in range(0, length, self.batch_sizeA):
                ...
                if (inc):
                    ...
                    if n_batch.shape[0] < self.batch_sizeA:
                        n_batch = np.concatenate(
                            (n_batch, inc_alin[:self.batch_sizeA - n_batch.shape[0]]), axis=0)
                    # n_batch = multiG.corrupt_align_batch(batch)
                else:
                    n_batch = multiG.corrupt_align_batch(batch)
                e1_batch, e2_batch, e1_nbatch, e2_nbatch = batch[:, 0], batch[:, 1], \
    ...
    def gen_AM_batch_non_neg(self, forever=False, shuffle=True):
        ...
        while True:
            ...
            for i in range(0, length, self.batch_sizeA):
                batch = align[i: i + self.batch_sizeA, :]
                if batch.shape[0] < self.batch_sizeA:
                    batch = np.concatenate((batch, align[:self.batch_sizeA - batch.shape[0]]),
                                           axis=0)
                    assert batch.shape[0] == self.batch_sizeA
    ...
    def train1epoch_KM(self, sess, num_A_batch, num_B_batch, a2, lr, epoch):
        ...
        print([l_ for l_ in this_loss])
        return this_total_loss

    def train1epoch_AM(self, sess, num_AM_batch, a1, a2, lr, epoch, am_fold):
    ...
    def train1epoch_associative(self, sess, lr, a1, a2, epoch, AM_fold=1, inc=[]):
        ...
        if epoch <= 1:
            print('num_KG1_batch =', num_A_batch,
                  " for total triples ", self.multiG.KG1.num_triples())
            print('num_KG2_batch =', num_B_batch,
                  " for total triples ", self.multiG.KG2.num_triples())
            print('num_AM_batch =', num_AM_batch,
                  " for total alignments ", self.multiG.num_align())
        loss_KM = self.train1epoch_KM(sess, num_A_batch, num_B_batch, a2, lr, epoch)
        # keep only the last loss
        for i in range(AM_fold):
            loss_AM = self.train1epoch_AM(sess, num_AM_batch, a1, a2, lr, epoch, i)
        return (loss_KM, loss_AM)

    def train_MTransE(self,
                      source,
                      target,
                      epochs=20,
                      save_every_epoch=10,
        ...
        for epoch in range(epochs):
            if half_loss_per_epoch > 0 and (epoch + 1) % half_loss_per_epoch == 0:
                lr /= 2.
            epoch_lossKM, epoch_lossAM = self.train1epoch_associative(
                self.sess, lr, a1, a2, epoch, AM_fold)
        ...
        if (len(inc) < 1):
            ...
            for epoch in range(10):
                epoch_lossKM, epoch_lossAM = self.train1epoch_associative(
                    self.sess, 0.001, a1, a2, epoch, 1, inc)
                print("Time use: %d" % (time.time() - t0))
                if np.isnan(epoch_lossKM) or np.isnan(epoch_lossAM):
                    print("Training collapsed.")
                    return
...
# A safer loading is available in Tester, with parameters like batch_size and dim recorded in the
